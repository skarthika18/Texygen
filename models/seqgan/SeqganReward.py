...
class Reward(object):
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        ...
        print("rewards",rewards,np.shape(rewards))
        reward_res = np.transpose(np.array(rewards)) / (1.0 * rollout_num)  # batch_size x seq_length
        print("reward_res",reward_res)
        print("shape",np.shape(reward_res))
        print("input_X : " , np.shape(input_x))
        # with sess:print(reward_res))
        return reward_res
